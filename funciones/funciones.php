...
//   define('IMAGE_PATH', dirname(__FILE__) . '/data/imagendeperfil/');
...
function validarRegistracion() {
  ...
  $contraseñaverificacion = trim($_POST["contraseñaverificacion"]);
  $imagenDePerfil = $_FILES["imagenDePerfil"];

  //Verificando que el nombre tenga más de 4 caracteres
  if ( empty($nombre)){
    $errores["nombre"] = "Debe completar el campo";
  }

  //Verificando que el telefono tenga 8 caracteres
  if (strlen($telefono) < 8){
    $errores["telefono"] = "El número no es válido";
  }

  //Verificando que el campo de contraseña no esté vacio
  if ( $contraseña == ""){
    $errores["contraseña"] = "Debe completar la contraseña";
  }

  //Verificando que el campo confirmacion de contraseña no esté vacio
  if ($contraseñaverificacion == ""){
    $errores["contraseñaverificacion"] = "Debe completar la confirmación del password";
  }

  //Verficando que las dos contraseñas sean iguales
  if( $contraseña != $contraseñaverificacion){
    $errores["contraseña_equivalentes"] = "Las contraseñas no coinciden";
  }
  ...
  if ( $imagenDePerfil["error"] === UPLOAD_ERR_OK ) {
      $errores["imagenDePerfil"] = "Subí una imagen";
    } else {
      // Si cargaron algún archivo, obtengo su extensión
      $ext = pathinfo($imagenDePerfil["name"], PATHINFO_EXTENSION);

      // Si la extesión del archivo que cargaron NO está en mi array de formatos permitidos
      if ( !in_array($ext, ALLOWED_IMAGE_FORMATS)){
        $errores["imagenDePerfil"] = "Los formatos permitidos son JPG, JPEG y PNG";
      }
  }
...
function saveImage() {
  ...
  $ext = pathinfo($imagenDePerfil["name"], PATHINFO_EXTENSION);
  $archivo = $imagenDePerfil["tmp_name"];

  $miarchivo = dirname(__FILE__);
  $miarchivo = $miarchivo . "/data/imagendeperfil/";
